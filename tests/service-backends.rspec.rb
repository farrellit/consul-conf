...
describe ConsulConf::ServiceBackends do
  ...
  it 'should return mocked health checks from getHealthChecks' do
    ...
    expect(res).to eq([{ 'Notes' => '',
                       ...
                       { 'Notes' => '',
                         ...
                         'ServiceID' => 'service1',
                         'Output' => '',
                         'CheckID' => 'service:service1',
                         'Node' => 'consul-client',
                         'Name' => "Service 'service1' check" }])
  ...
  it 'should return expected node from getServiceNodes for healthy service' do
    expect(sb.getServiceNodes 'service1').to eq(
    [{ 'name' => 'consul-client',
       'ip' => '10.1.2.3',
       'port' => 80,
       'status' => 'up',
  ...
  it 'should return expected node from getServiceNodes for unhealthy service' do
    expect(sb.getServiceNodes 'service2').to eq(
[{ 'name' => 'consul-client',
   'ip' => '10.1.2.3',
   'port' => 81,
   'status' => 'down',
  ...
  it 'should return all services from allServiceBackends' do
    expect(sb.allServiceBackends).to eq(
[{ 'name' => 'service1',
   'port' => 8080,
   'cookie' => true,
   'check' => 'inter 3000 rise 2 fall 3 maxconn 255',
   'options' => ['httpchk GET /ping'],
   'servers' =>    [{ 'name' => 'consul-client',
                      'ip' => '10.1.2.3',
                      'port' => 80,
                      'status' => 'up',
                      'health_checks' =>       [{ 'Notes' => '',
 ...
 { 'name' => 'service2',
  ...
  it 'should return all up ervers from allServiceBackends(true)' do
    expect(sb.allServiceBackends true).to eq(
[{ 'name' => 'service1',
   'port' => 8080,
   'cookie' => true,
   'check' => 'inter 3000 rise 2 fall 3 maxconn 255',
